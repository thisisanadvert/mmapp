/*
  ...
  1. Changes
    ...
    - Fix infinite recursion in building_users table
    - Simplify policy logic for better performance
    - Use direct ID comparisons instead of self-referential queries
...
BEGIN
  IF NOT EXISTS (
    ...
    AND policyname = 'Users can view announcements in their buildings'
  ) THEN
...
BEGIN
  IF NOT EXISTS (
    ...
    WHERE schemaname = 'public' 
    AND tablename = 'polls'
    AND policyname = 'Users can view polls in their buildings'
  ) THEN
...
BEGIN
  IF NOT EXISTS (
    ...
    AND policyname = 'Users can read documents from their buildings'
  ) THEN
    ...
    USING (
      bucket_id = 'documents' AND (
        ...
        (EXISTS (
          SELECT 1 FROM auth.users
          WHERE id = auth.uid() AND id IN (
            SELECT user_id FROM public.building_users 
  ...
  IF NOT EXISTS (
    ...
    WHERE schemaname = 'storage' 
    AND tablename = 'objects'
    AND policyname = 'Directors can upload documents'
  ) THEN
