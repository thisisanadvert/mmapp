...
BEGIN
  ...
  -- Check for demo building users access policy
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies 
    WHERE schemaname = 'public' 
    AND tablename = 'building_users'
    AND policyname = 'Allow access to demo building users'
  ) THEN
    -- Create a comprehensive demo building users access policy
    CREATE POLICY "Allow access to demo building users"
      ON building_users
      FOR ALL
      TO authenticated
      USING (
