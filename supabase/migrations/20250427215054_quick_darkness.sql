...
SELECT
  ...
  END,
  CASE 
    WHEN u.raw_user_meta_data->>'role' = 'rtm-director' THEN (SELECT id FROM units WHERE unit_number = '2A' LIMIT 1)
    WHEN u.raw_user_meta_data->>'role' = 'leaseholder' THEN (SELECT id FROM units WHERE unit_number = '1B' LIMIT 1)
    ELSE NULL
  END
FROM auth.users u
