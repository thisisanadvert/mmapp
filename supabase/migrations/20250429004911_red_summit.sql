/*
  ...
  1. New Tables
    - `onboarding_status`
      ...
      - Stores completion status of each step

  2. Functions
    - `create_initial_rtm_setup`: Creates initial building and user records
    - `handle_new_rtm_signup`: Trigger function for new RTM signups
...
RETURNS trigger AS $$
BEGIN
  IF NEW.raw_user_metadata->>'role' = 'rtm-director' THEN
    PERFORM create_initial_rtm_setup(
      NEW.id,
      NEW.raw_user_metadata->>'buildingName',
      NEW.raw_user_metadata->>'buildingAddress',
      NEW.raw_user_metadata->>'unitNumber'
    );
  END IF;
  RETURN NEW;
END;
